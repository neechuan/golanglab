...
func main() {
    ...
    for btCount < maxBtCount {
        ...
        backendName := "EUMCollector"
        backendType := "HTTP"
        backendProperties := map[string]string{"URL": "http://ip-10-97-28-69.us-west-2.compute.internal:7001/eumcollector/get-versions"}
        resolveBackend := false
        appd.AddBackend(backendName, backendType, backendProperties, resolveBackend)

        // Start AppD BT
        btHandle2 := appd.StartBT("CheckEUM", "")
        ecHandle := appd.StartExitcall(btHandle2, backendName)
        hdr := appd.GetExitcallCorrelationHeader(ecHandle)
        response, err := http.Get("http://ip-10-97-28-69.us-west-2.compute.internal:7001/eumcollector/get-versions")
	...
	fmt.Println(hdr)
		...
		fmt.Println("AddBTError")
                ...
                appd.AddUserDataToBT(btHandle2, "Error", "EUM Collector or API is unreachable")
                appd.SetBTURL(btHandle2, "/checkeum")
        }
        if err != nil {
		fmt.Print("error: ")
